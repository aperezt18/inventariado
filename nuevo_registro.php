...
function esUbicacionValida($ubicacion) {
    ...
    if (in_array($ubicacion, ['PTAG', 'ALMACÉN', 'TUNEL'])) return true;
    return false;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($tipo !== 'LIBRE') {
        ...
        if ($tipo_maquina === 'DCP' && !in_array($tipo, ['P', 'PTAG'])) {
        ...
        if ($estado !== '' && in_array($estado, ['Stock', 'Reparación', 'Garantía']) && $ubicacion !== 'ALMACÉN') {
    ...
    if ($tipo !== 'LIBRE' && empty($errores)) {
        ...
        } elseif ($tipo_maquina !== 'BGR' && $count_tipo > 0) {
